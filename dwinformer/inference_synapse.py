...
def test(fold):
    path='/home/daniya/nn2/dwinformer/DATASET/dwinformer_raw/dwinformer_raw_data/Task002_Synapse/'
    label_list=sorted(glob.glob(os.path.join(path,'labelsTs','*nii.gz')))
    infer_list=sorted(glob.glob(os.path.join(path,'ax',fold,'*nii.gz')))
