...
import argparse
from dwinformer.preprocessing.preprocessing import resample_data_or_seg
...
if __name__ == "__main__":
    ...
    trainer_class = recursive_find_python_class([join(dwinformer.__path__[0], "training", "network_training")],
                                                trainerclass,
                                                "dwinformer.training.network_training")

    if trainer_class is None:
        raise RuntimeError("Could not find trainer class in dwinformer.training.network_training")
    else:
        assert issubclass(trainer_class,
                          dwinformerTrainer), "network_trainer was found but is not derived from dwinformerTrainer"
