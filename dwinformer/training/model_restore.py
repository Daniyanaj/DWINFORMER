...
import importlib
import pkgutil
from dwinformer.training.network_training.dwinformerTrainer import dwinformerTrainer
from dwinformer.training.network_training.dwinformerTrainer_synapse import dwinformerTrainer_synapse
from dwinformer.paths import network_training_output_dir, preprocessing_output_dir, default_plans_identifier
...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None,folder=None):
    """
    This is a utility function to load any dwinformer trainer from a pkl. It will recursively search
    dwinformer.trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    is specified, it will furthermore load the checkpoint file in train/test mode (as specified by train).
    The pkl file required here is the one that will be saved automatically when calling dwinformerTrainer.save_checkpoint.
    :param pkl_file:
    :param checkpoint:
    :param train:
    ...
    if 'nnUNet' in name:
        name=name.replace('nnUNet','dwinformer')
    if len(init)>10:
        init=list(init)
        del init[2]
        del init[-2]
    search_in = join(dwinformer.__path__[0], "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="dwinformer.training.network_training")
    ...
    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in dwinformer.trainig.network_training. If it "
                           "is not located there, please move it or change the code of restore_model. Your model "
                           "trainer can be located in any directory within dwinformer.trainig.network_training (search is recursive)."
                           "\nDebug info: \ncheckpoint file: %s\nName of trainer: %s " % (checkpoint, name))
    assert issubclass(tr, dwinformerTrainer) or issubclass(tr, dwinformerTrainer_synapse), "The network trainer was found but is not a subclass of dwinformerTrainer.Please make it so!"

    # this is now deprecated
    """if len(init) == 7:
        print("warning: this model seems to have been saved with a previous version of dwinformer. Attempting to load it "
...
if __name__ == "__main__":
    pkl = "/home/fabian/PhD/results/dwinformerV2/dwinformerV2_3D_fullres/Task004_Hippocampus/fold0/model_best.model.pkl"
    checkpoint = pkl[:-4]
    train = False
